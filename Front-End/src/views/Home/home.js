...
const Home = () => {
  ...
  return (
    <div className="home-container">
      ...
      <p>Želite živeti bolj zdravo življenje ampak ne veste kje začeti in potrebujete pomoč? Predlagamo da preizkusite novo orodje:</p>
      <div className="link-button">
        <Link to="/chat" className="quick-link">AI Klepet</Link>
      </div>
          ...
          {featuredTrainings.length === 0 ? (
            ...
            featuredTrainings.map((training) => (
              <div key={training._id} className="card-user">
                ...
                <p><strong>Kalorije:</strong> {training.total_calories}</p>
              </div>
            ))
          )}
        </div>
          ...
          {featuredExercises.length === 0 ? (
            ...
            featuredExercises.map((exercise) => (
              <div key={exercise._id} className="card-user">
                ...
                <p><strong>Kalorije:</strong> {exercise.calories}</p>
              </div>
            ))
          )}
        </div>
      ...
      <section className="motivational-section" style={{ backgroundColor: 'white', border: '3px solid #FED467' }}>
        <h1>Motivacijski kotek</h1>
        <p><i>"The only bad workout is the one that didn’t happen."</i></p>
        <br></br>
      </section>
    </div>
  );
